...
r5_color_dat <- list(subj_prior=c(0,0,0,0),
  item_prior=c(0,0,0,0),
  subject_id=as.integer(factor(r5$subject.index)),
  item_id=as.integer(factor(r5$item)),
...
modr5_color <- stan(model_code = color_stan,
            data = r5_color_dat,
            iter = 1500, warmup=750, chains = 4,
            pars=c("beta_f1","beta_f2",
                   "beta_f1_X_f2",
...
<<>>=
print(modr5_color)
MCMCsamp<-as.matrix(modr5_color)
...
r13_color_dat <- list(subj_prior=c(0,0,0,0),
  item_prior=c(0,0,0,0),
  subject_id=as.integer(factor(r13$subject.index)),
  item_id=as.integer(factor(r13$item)),
...
modr13_color <- stan(model_code = color_stan,
            data = r13_color_dat,
            iter = 1500, warmup=750, chains = 4,
            pars=c("beta_f1","beta_f2",
                   "beta_f1_X_f2",
...
<<>>=
print(modr13_color)
MCMCsamp<-as.matrix(modr13_color)
...
The probabilities in the last column in Table 2:
